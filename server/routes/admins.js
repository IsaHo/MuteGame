...
// Whoami — used by the admin UI to populate sidebar / topbar with the
// logged-in operator's username + display name.
router.get('/me', requireAdmin(), (req, res) => {
  const db = getDb();
  const row = db.prepare('SELECT id, username, role, permissions, is_active, display_name FROM admins WHERE id = ?').get(req.admin.id);
