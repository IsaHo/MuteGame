...
router.delete('/items/:id', adminIpGuard, (req, res) => {
  ...
  if (referenced > 0) {
    db.transaction(() => {
      ...
      audit(req, 'shop.item.delete', 'shop_item', id, { mode: 'soft', referencedBy: referenced });
    })();
    broadcastShopItems(req);
  ...
  db.transaction(() => {
    ...
    audit(req, 'shop.item.delete', 'shop_item', id, { mode: 'hard' });
  })();
  broadcastShopItems(req);
...
// Old behavior silently approved orders when credits were insufficient
// (revenue leak). C3B switches to strict mode: InsufficientFundsError
// rolls back the whole transaction (stock decrement included) and
// returns HTTP 400. Operator must then either top up the customer's
// credits or change payment_method to 'cash'.
router.post('/orders/:id/approve', adminIpGuard, (req, res) => {
  ...
  try {
    db.transaction(() => {
      ...
      // Deduct credits if credits payment. Strict mode → throws
      // InsufficientFundsError which bubbles up and rolls back the
      // entire transaction (including the stock decrement above).
  ...
  // Emit AFTER commit so we don't broadcast a rolled-back approval.
  if (chargeResult) {
